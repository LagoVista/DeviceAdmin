...
namespace LagoVista.IoT.DeviceAdmin.Tests.ValidationTests
{
    ...
    public class UnitSetValidation : ValidationTestBase
    {
        private UnitSet GetUnitSet()
        {
            ...
            unitSet.Units.Add(new Unit() { Key = "state1", Name = "state1", IsDefault = true, Abbreviation = "s" });
            unitSet.Units.Add(new Unit() { Key = "state2", Name = "state1", IsDefault = false, ConversionType = EntityHeader<ConversionTypes>.Create(ConversionTypes.Factor), Abbreviation="s2", ConversionFactor = 1.5 });
        ...
        public void UnitSet_No_DefaultUnit_InValid()
        {
            ...
            unitSet.Units[0].ConversionFactor = 1.5;
